...
const referralSummaryStatsRetriever = async () => {
  ...
  const summaryTable = $('#overview-referral-promo-summary')
  const ownerSummaryCount = results.owner_summary ? numeral(results.owner_summary.length).format('0,0') : '0'
  ...
  const platformTable = $('#overview-referral-promo-platform tbody')
  let sortedPlatforms = results.platform_summary.sort((a, b) => {
    return (b.downloads || 0) - (a.downloads || 0)
  }).filter((a) => {
    return a.downloads > 10
  })
...
const tbl = $('#overview-referral-promo-top-channels tbody')
const fillChannels = () => {
  tbl.empty()
...
$("#referral-download").on('click', async (evt) => {
  ...
  COMMON.downloadObjectAs(buffer, 'summary.csv', 'text/csv')
})

module.exports = referralSummaryStatsRetriever
