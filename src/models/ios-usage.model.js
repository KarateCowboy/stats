...
module.exports = function () {
  ...
  let IosUsage = new Schema({
    ...
    platform: {
      ...
      required: true,
      validate: {
        validator: function (v) { return ['ios'].includes(v) }
      }
    },
    version: {
      type: Schema.Types.String,
    ...
    timestamps: true,
    collection: 'ios_usage'
  })
  AttachCommonMethods(IosUsage)

  // put more instance and static methods after here
  IosUsage.virtual('aggregate_collection').get(() => 'ios_usage_aggregate_woi')

  if (mongooseClient.models.IosUsage) {
    return mongooseClient.models.IosUsage
  } else {
    return mongooseClient.model('IosUsage', IosUsage)
  }
}
