...
class FTI {
  ...
  static async archive (object_id) {
    try {
      const results = await pg.query(FIND_BY_OBJECT_ID, [object_id])
      const fti = results.rows[0]
      await pg.query(INSERT_INTO_ARCHIVE, [fti.id, fti.object_type, fti.object_id, fti.searchable])
