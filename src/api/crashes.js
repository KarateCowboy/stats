...
exports.setup = (server, client, mongo) => {
  ...
  async function reindexCrash (client, id, done) {
    await client.query('UPDATE dtl.crashes SET id = id WHERE id = $1', [id], done)
  }
  ...
  server.route({
    ...
    handler: async function (request, h) {
      await client.query(POST_TAG, [request.params.id, request.params.tag])
      await reindexCrash(client, request.params.id)
  ...
  server.route({
    ...
    handler: async function (request, h) {
      await client.query(DELETE_TAG, [request.params.id, request.params.tag])
      await reindexCrash(client, request.params.id)
      return h.response({status: 'ok'})
    }
  })
