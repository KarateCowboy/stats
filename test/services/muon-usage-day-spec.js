...
let MuonUsageDayService = require('../../src/services/muon-usage-day.service')
const {ObjectID} = require('mongodb')

let service
describe('muon-usage-day service', async function () {
  describe('#retention_spread', async function () {
    beforeEach(async function () {
      ...
      for (let i in _.range(1, 13)) {
        for (let ref of refs) {
          const usage_day = await factory.build('muon_usage_day')
