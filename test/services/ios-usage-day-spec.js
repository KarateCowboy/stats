...
let IosUsageDayService = require('../../src/services/ios-usage-day.service')
const {ObjectID} = require('mongodb')

let service
describe('ios-usage-day service', async function () {
  describe('#retention_spread', async function () {
    beforeEach(async function () {
      ...
      for (let i in _.range(1, 13)) {
        for (let ref of refs) {
          const usage_day = await factory.build('ios_usage_aggregate_woi')
